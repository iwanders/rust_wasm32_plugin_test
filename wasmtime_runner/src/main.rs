...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    linker.func_wrap("env", "log_record", |mut caller: Caller<'_, MyStorage>, ptr: i32, len: i32| {
        // println!("Pointer: caller: {caller:?}");
        println!("Pointer: {ptr:?}, len: {len}");

        let mem = match caller.get_export("memory") {
            Some(Extern::Memory(mem)) => mem,
            _ => {println!("failed to find host memory"); return;},
        };
    ...
    // run it.
    let sum = instance.get_func(&mut store, "sum")
    ...
    for i in 0..1000000i32 {
        let result = match sum_fun.call(&mut store, (i, i)) {
            Ok(v) => v,
            Err(z) => {println!("z: {z:?}"); break;},
        };
        // println!("Answer: {:?}", result);
    }

    // Call the imported 'foo' symbol.
    let call_foo = instance.get_func(&mut store, "call_foo")
    ...
    {
        // log test
        let log_setup = instance.get_func(&mut store, "log_setup")
            .expect("`call_foo` was not an exported function");
        log_setup.call(&mut store, &[], &mut [])?;
        let log_test = instance.get_func(&mut store, "log_test")
    ...
    {
        let prepare_input = instance.get_func(&mut store, "prepare_input").expect("should have this");
        ...
        let use_input = instance.get_func(&mut store, "use_input").expect("use-input not found");
    ...
    // test sin
    {
        let test_sin = instance.get_func(&mut store, "test_sin").expect("test_sin missing");
